...
class FileFinder(QtCore.QObject):
    ...
    def _filter_work_files(self, work_file_paths, valid_file_extensions):
        ...
        for item in hook_result:
            ...
            file_details["metadata"] = work_file.get("metadata", {})

            work_files.append(file_details)

        return work_files

class PerforceFileFinder(FileFinder):

    def _get_template_glob_str(self, template, fields, skip_keys=None, skip_missing_optional_keys=False):
        # Taken from sgtk.api.paths_from_template
    ...
    def _find_work_files(self, context, work_template, version_compare_ignore_fields):
        ...
        # This should be moved to a hook somehow.


        # find work files that match the current work template:
        work_file_paths = super(PerforceFileFinder, self)._find_work_files(context, work_template, version_compare_ignore_fields)
        work_fields = []
        try:
            work_fields = context.as_template_fields(work_template, validate=True)
        except TankError:
        ...
        # get_templte_glob_str code is from core.api.paths_from_template.
        # Perhaps could be a method on the Template class?
        glob_str =  self._get_template_glob_str(work_template,
                                                work_fields,
                                                skip_fields,
                                                skip_missing_optional_keys=True)

        # Find workfiles that may be in Perforce.

        from oss import source_control
        sc = source_control.get_instance()

        with sc._p4.at_exception_level(source_control.RAISE_ERRORS):
            # For some reason run_files returns False if no files are found
            # rather than an empty list...
            files_in_depot = sc._p4.run_files(glob_str) or []

        local_file_paths = sc.depot_to_local_paths([f["depotFile"] for f in files_in_depot])
        for work_file in local_file_paths:
            work_file = os.path.normpath(work_file)
            if work_file not in work_file_paths:
                work_file_paths.append(work_file)

        return work_file_paths


class AsyncFileFinder(PerforceFileFinder):
    """
